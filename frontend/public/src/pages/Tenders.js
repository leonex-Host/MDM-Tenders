...
export async function renderTenders(container) {
    ...
    const renderTable = (dataToRender) => {
        ...
        const maxLen = dataToRender.length;

        function drawGrid() {
            const hasMoreClient = visibleCount < maxLen;
            const hasMoreBackend = allTenders.length < totalAvailable;
            const needsMore = hasMoreClient || hasMoreBackend;

            const slice = dataToRender.slice(0, visibleCount);
            ...
            // Bind Load More natively mapping either Local RAM boundaries or DB Offsets
            area.querySelector('#client-load-more-btn')?.addEventListener('click', () => {
                if (hasMoreClient) {
                    ...
                    visibleCount += 100; // prep buffer for newly fetched items
                    loadData(true);
                }
            });
            ...
            area.querySelectorAll('.bookmark-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    if (isNowSaved) b.classList.add('active');
        ...
        drawGrid(); // Launch physical DOM wrap
    };
